...
class Command(BaseCommand):
    """
        Api call to fetch cities and hotels MANUALLY via command line
        two blocks: to fetch data from city api and hotel api with command:
        for cities:  $python manage.py  get_api_mix (or python manage.py  get_capi_mix --fetch city)
        for hotels:  $python manage.py  get_api_mix --fetch hotel
